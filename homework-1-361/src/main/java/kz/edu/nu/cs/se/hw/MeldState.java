...
public class MeldState extends StepsStates {

    private Rummy rummy;

    public MeldState(Rummy rummy) {
        this.rummy = rummy;
    }
    ...
    public void meld(String... cards) {
        if(this.rummy.getPlayerHands().get(this.rummy.getCurrentPlayer()).size() == 0) {
            ...
            this.rummy.setCanDeclareRummy(false);
        }
    }
    ...
    public void declareRummy() {
        if (this.rummy.isCanDeclareRummy()) this.rummy.setCurrentStep(this.rummy.rummy);
    }

    @Override
    public void finishMeld() { this.rummy.setCurrentStep(this.rummy.discard); }

    @Override
    public void discard(String card) { throw new RummyException("Expected Discard step", 6); }

    @Override
    public Steps getStepName() { return Steps.MELD; }
}
